...
\end{enumerate}
	∫\underbrace{[\frac{f(u+εv,u'+εv')-f(u,u')}_{g_ε(x)}ε]}\dd{x}
...
\sim \pdv_zf(\bar u,\bar u') \frac{\bar u'(\frac 1{1+εφ'}-1)}ε

\rightarrow 
-\pdv_zf(\bar u,\bar u')\bar u'φ'

\lim_{ε\to 0}\frac{F(u_ε)-F(u)}ε=∫_0^1[-\pdv_zf(\bar u,\bar u')\bar u'φ'+f(\bar u,\bar u')φ']\dd{x}
...
Donc φ\in C^∞ φ(0)=φ(1)=0 $\implies$ ∫_0^1[f(\bar u,\bar u')-\pdv_zf(\bar u,\bar u')]φ'\dd{x}=0

$\implies$ ∫_0^1\pdv{x} (\pdv_zf(\bar u,\bar u')\bar u'-f(\bar u,\bar u'))φ=0
...
\label{par:etape_1}
\exists \tilde f: X\rightarrow ]-∞,+∞]\ |\ \overline{\epi f}=\epi \tilde f (\tilde f est donc s.c.i.) A=\overline{\epi f} est un fermé de X\times\R. Soit (x,α)\in A et β>α. Alors \exists (x_n,α_n)\in\epif\ |\ x_n\to et α_n\toα
